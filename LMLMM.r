...
require(plyr)
ages <- ddply(rawData,~Sub_id,summarize,Age=mean(Age), Sex = paste(unique(Sex), collapse = ""))
mAge <- round(mean(ages$Age), digits =2)
sdAge <- round(sd(ages$Age), digits =2)
nGdr <- table(ages$Sex)

rm(ages, mAge, sdAge, nGdr) #Ta bort onödiga values när allt är ned sparat

#Accuracy descriptives
#1 Genom hela experimentet
xAcc <- ddply(rawData,~Sub_id,summarize,Accuracy=mean(Accuracy)) #Generell acc
for(i in levels(rawData$Modality)){
  xmAcc <- ddply(rawData,~Sub_id,summarize,i=mean(Accuracy)) #Divided in blocks fungerar ej
}
